...
# --- Suppress cache_all Deprecation Warning ---
class FilterCacheAllWarning(logging.Filter):
    def filter(self, record):
        return "cache_all is deprecated" not in record.getMessage()


logging.getLogger().addFilter(FilterCacheAllWarning())
logging.getLogger("root").addFilter(FilterCacheAllWarning())
# ----------------------------------------------------------
...
def universal_tokenizer_patch(orig_fn):
    def patched_fn(pretrained_model_name_or_path, *args, **kwargs):
        ...
        kwargs['use_fast'] = False  # Set to false to avoid issues with some fast tokenizers
...
def get_validation_data(num_examples=None, seq_len=None, device=None):
    # Warning: This data is tokenized for GPT-2. It will cause issues with Llama-3.
...
def main():
    ...
    # ---------------------------------------------------------
    # RESUME LOGIC (Load from previous run)
    # ---------------------------------------------------------
